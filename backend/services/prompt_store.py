...
对每条规则输出一个审核结论,返回 JSON:
{
  "findings": [
    {
      "rule_id": "规则ID",
      "detail": "判定理由,必须按【定位→取证→校验→结论】四步书写(见下方【推理过程书写规范】):①定位(指出核查文件+章节/段落/行号/字段名)②取证(逐字摘录原文)③校验(数值/日期写完整算式与结果)④结论(明确 pass/fail/warn/unknown,且与 status 一致)",
      "title": "一句话结论(必须与 detail 的结论一致)",
      "status": "pass|fail|warn|unknown",
      "evidence": "支撑结论的原文逐字摘录(10-200字),须与原始材料逐字一致,不得改写/概括/意译;较长原文可在首尾以「……」省略无关部分(中间不得跳跃省略,省略不改语义);注明所在文件与精确位置(章节/段落/行号/字段名);status=pass 时留空;严禁填「无/暂无/未找到」等占位词;严禁填入结构化字段串(如 `{title:..., reason:...}`)——evidence 字段只承载原文逐字摘录,字段结构化信息请写在 detail 中按四步组织",
...
【推理过程书写规范(分步·因果明确)】
- 每条规则的 detail 必须严格按「定位 → 取证 → 校验 → 结论」四步顺序书写,每步独立成句,因果链清晰可读:
  ① 定位:先指出本次核查落在哪个文件的哪个具体位置(章节/段落/行号/字段名),让结论可回溯,禁止以"相关条款""前述内容"等指代代替位置。
  ② 取证:在该位置逐字摘录支撑判定的原文(见【原文引用规范】),不得省略原文只写结论。
  ③ 校验:数值/日期/比例类必须写出完整算式与计算过程及结果(如:大写壹佰万元=1,000,000元;差值=1,000,000−980,000=20,000元);逻辑类须写明比对的两边与差异。
  ④ 结论:基于①②③推出唯一结论词(pass/fail/warn/unknown),结论须能从上文直接推出,不得突兀翻转。
- 严禁先下结论再倒找理由(那会导致 status 与 detail 矛盾被系统降级);严禁用"经评审/经核查"等无推理模板句替代分步说明。
- 多要素规则逐要素给出上述四步,要素之间以序号或空行分隔,避免混为一谈、因果不清。
