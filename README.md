...
- **全自动流程**：上传法规文件后自动发起抽取并关闭弹窗，进度在规则集卡片实时滚动；解析未完成时即可创建审核任务，系统自动**串行衔接**——先在审核任务中实时镜像解析进度，解析完成后无缝切换为合规审核进度，无需等待
- **解析结果复用**：生成前按「来源+参数指纹 > 文件 MD5 集合 > 法规版本信息（法规名+文号）」三级匹配已有解析结果，命中直接复用并留痕，跳过重复解析、不重复消耗 token
- 默认抽取规则条数可配置（`legal_max_rules`，默认 30，见服务配置「法规解析」分区）
- 取消「等待法规解析」中的审核任务时会有明确提醒：仅终止审核，法规解析在后台继续完成
...
- **全局交互规范**：滚动时全局头部与各页顶栏行（标题/返回/主要操作）固定吸顶，随时可见可操作；页面右下角有「回到顶部」悬浮按钮（滚动超一屏出现，点击平滑回顶），全部页面表现一致，窄屏自适应
...
| OCR | 图聆云图文识别（默认）/ 百度智能云 OCR（AK/SK 授权）/ 讯飞开放平台通用文字识别 intsig（AppID+APIKey+APISecret 签名），可配置切换 |
...
> **OCR 服务可选三种**（「服务配置 → OCR 服务 → 服务类型」切换，改后即测即生效）：
...
> 需要基于某部法规生成专属规则？在规则面板选择「按法规文件」→ 上传法规文件即可——系统自动开始解析（约 5 分钟/万字）并关闭弹窗，进度在下方实时滚动。解析期间就能创建审核任务，系统会自动先完成解析再开始审核；相同法规再次上传会直接复用已有解析结果。
