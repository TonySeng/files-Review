...
GENERAL_RULES: list[dict[str, Any]] = [
    {
        ...
        "description": (
            ...
            "与标点符号规范。错别字指实际用词错误（如『投标保障金』应为『投标保证金』），"
            "须与标点/格式问题区分。"
        ),
        "checkpoints": [
            "逐句扫描，重点识别形近字、音近字被错误替换（如『保障金』应为『保证金』）",
            "识别多字、漏字或字序颠倒导致的表意错误",
            "标点符号使用是否规范（中英文符号混用、标点缺失、多余空格等）",
            "专业术语拼写/用词是否正确",
            "错别字与标点/格式问题须分别判定；未识别到错别字时 status 判为 pass，不要把标点噪声当作错别字",
        ],
    },
    {
        "id": "gen-semantics", "name": "语义通顺性与逻辑一致性",
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "检查语句是否通顺、表述是否清晰、逻辑是否自洽。",
        "checkpoints": [
            "是否存在语句不通、表意不明",
            "是否存在逻辑矛盾或前后冲突",
            "是否存在主谓不一致、成分残缺",
    ...
    },
    {
        ...
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "专业术语使用规范统一，表述方式符合行业惯例。",
        "checkpoints": [
            "专业术语使用是否规范",
            '同一概念的表述是否统一（例如："投标人"与"供应商"混用）',
            "缩写是否在首次出现时注明全称",
        ],
    },
    {
        ...
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "数字、日期、金额的表示方式应统一规范。",
        "checkpoints": [
            "数字使用是否统一（阿拉伯数字与中文数字）",
    ...
    {
        "id": "gen-toc", "name": "文档结构完整性（目录、页码）",
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "目录、页码、章节编号应完整准确。",
        "checkpoints": [
            "目录是否与正文章节对应",
            "页码是否连续且与目录一致",
            "章节编号是否连贯无遗漏",
    ...
    },
    {
        ...
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "引用法规、标准、文献时应准确标注出处。",
        "checkpoints": [
            "引用的法律法规名称、条款编号是否准确",
            "引用的技术标准编号与版本是否正确",
            "图表、数据来源是否标注",
    ...
    },
    {
        ...
        "category": "general_quality", "severity": "minor", "need_legal_basis": False,
        "description": "正文引用的附件应齐全，附件编号与正文一致。",
        "checkpoints": [
            "正文提及的附件是否全部提供",
            "附件编号、名称与正文引用是否一致",
            "附件内容是否与正文描述匹配",
