...
8. 要素来源与抽取纪律(反幻觉核心):
   8.1 任何结论(尤其 fail/warn)中提到的文件、要素取值,必须来自上方"待审文件内容"区块中
       真实出现的文件;严禁引用本次未上传的文件(如文档里没有的"XX公司投标文件""补充承诺函"),
       这类无来源引用会被系统直接降级为待人工复核.
   8.2 一致性类规则(dec-09/dec-10/dec-11/dec-12 等)只能比对"确实从对应文件抽取到的"要素:
   ...
   8.3 工期/交付期/投标有效期/质保期等时间字段:必须逐字摘录其原文表述与所在文件,
       不得用"合理工期""通常做法"等推断值替代;原文未写明该字段时该要素直接跳过,不审查.
   8.4 每条 fail/warn 的 evidence 必须显式标注来源文件(如"见《XX投标函》第2页");
       若只能对应到泛称(如"投标文件"),须在 involved_files 填该真实文件名,禁止写
       "附件1和附件2"等无法定位的泛称.involved_files 只能从待审文件真实文件名中选择.
   8.5 未实际抽取到的要素:在 detail 中标注"【跳过:原文未提及XX要素】",不得输出针对该
